...
This describes four facts about the same subject. You can see this exact Turtle output on any DanNet RDF resource page. For instance, download the [Turtle file for synset-52](/dannet/data/synset-52?format=text/turtle).
...
> **NOTE:** the "Run this query" link above has **label enrichment** enabled). This means the editor automatically maps labels to any resource URIs in the results, so you don't need to manually fetch `rdfs:label` for every variable. The **?synset** column will show clickable, labelled links.

### Climbing the hypernym hierarchy

You can also use `+` in a [property path](/dannet/page/sparql#property-paths) to mean "one or more steps". This query finds all ancestors of "delfin" (dolphin, [synset-3346](/dannet/data/synset-3346)), all the way up to the root:
...
DanNet annotates every synset with one or more ontological types from the `dnc:` namespace. You can get an overview of all available types and how many synsets each one covers:
...
With enrichment enabled, both the DanNet synset and the English synset columns will show labelled links, making it easy to see which Danish meanings map to which English ones.
...
- **Source** selects the inference mode: *Auto* (default) lets the server decide, *Raw* queries only the raw triplestore (faster, but can require writing complex queries), and *Inferred* forces the inference model (slower, but includes triples derived via logical inference).
...
Results are displayed as a table with clickable resource links. If the result set exceeds the page size, previous/next buttons appear. If the query contains a `LIMIT` or `OFFSET`, the editor disables the automatic pagination feature and lets you do this manually instead.
